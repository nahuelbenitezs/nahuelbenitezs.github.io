...
const vec3 PINK  = vec3(233.0, 71.0, 245.0) / 255.0; // Magenta
const vec3 BLUE  = vec3(47.0,  75.0, 162.0) / 255.0; // Azul oscuro
...
vec3 background_color(vec2 uv) {
  ...
  col += mix(BLUE * 0.5, BLACK, smoothstep(0.0, 1.0, abs(m)));
  return col * 0.3; 
}

vec3 getLineColor(float t, vec3 baseColor) {
  if (lineGradientCount <= 0) {
    return mix(PINK, BLUE, t);
  }
...
const FloatingLines = ({
  ...
  useEffect(() => {
    ...
    const newLinesGradient = [
        "#4D4DFF", // Azul vibrante (similar al inicio del gradiente en tu imagen)
        "#8A2BE2", // Azul violeta (similar al centro)
        "#E22BE2", // Magenta vibrante (similar al final)
        "#4D4DFF"  // Repite el azul para un loop más suave si es necesario, o déjalo en 3
      ];

    if (newLinesGradient && newLinesGradient.length > 0) {
      const stops = newLinesGradient.slice(0, MAX_GRADIENT_STOPS);
    ...
    topLineCountVal, middleLineCountVal, bottomLineCountVal,
    topLineDistanceVal, middleLineDistanceVal, bottomLineDistanceVal
  ]);
